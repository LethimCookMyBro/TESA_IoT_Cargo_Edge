...
export const FleetGuardianScene: React.FC = () => {
  const frame = useCurrentFrame();

  // Timeline sub-phases (Total 660 frames / 22 seconds):
  // Phase 1: 0 - 220 (Fleet Overview)
  // Phase 2: 220 - 440 (Safety Events & Mission History Pagination)
  // Phase 3: 440 - 660 (Maintenance Assistant & Hermes Boundary)

  let activeImage = "assets/current-capture/11_fleet_overview.png";
  let statusBadge = "FLEET OVERVIEW";
  let badgeColor = "#00f2fe";
  let calloutText = "ติดตามหลายหุ่นยนต์ และแยกสถานะความปลอดภัยรายตัว";
  ...
  if (frame >= 220 && frame < 440) {
    ...
    badgeColor = "#3b82f6";
    calloutText = "บันทึก Safety Events และประวัติภารกิจย้อนหลังลง PostgreSQL แบบ Non-blocking";
  } else if (frame >= 440) {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0b0f19", color: "#ffffff" }}>
      ...
      >
        ...
        {/* Top Floating Badges Bar (Critical for Hermes requirement) */}
        {frame >= 440 && (
          <div
            style={{
              position: "absolute",
