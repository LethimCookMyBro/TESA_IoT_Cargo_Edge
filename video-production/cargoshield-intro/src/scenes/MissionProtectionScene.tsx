...
export const MissionProtectionScene: React.FC = () => {
  const frame = useCurrentFrame();

  // Timeline sub-phases (Total 1020 frames / 34 seconds):
  // Phase 1: 0 - 240 (MOVING & Fragile Cargo)
  // Phase 2: 240 - 540 (Camera Views: Overview / Follow / Robot POV)
  // Phase 3: 540 - 780 (HOLD UNCERTAIN state)
  // Phase 4: 780 - 1020 (SLOW DOWN & SAFE STOPPED intervention)

  const currentPhase =
    frame < 240 ? 1 : frame < 540 ? 2 : frame < 780 ? 3 : 4;

  // Image source selection based on sub-phase
  let activeImage = "assets/current-capture/03_mission_moving.png";
  let statusBadge = "MOVING";
  let statusColor = "#10b981";
  let calloutText = "เลือกสินค้าเปราะบาง ➔ นโยบายความเร็วปรับลดความเร็วระมัดระวัง";

  if (currentPhase === 1) {
    activeImage = "assets/current-capture/02_mission_fragile_cargo.png";
    statusBadge = "FRAGILE CARGO POLICY";
    statusColor = "#3b82f6";
    calloutText = "สินค้าเปราะบาง (Fragile Cargo) ถูกจำกัดความเร็วเข้มงวดกว่าสินค้าทั่วไป";
    ...
    statusColor = "#00f2fe";
    calloutText = "3D Mission Console: สลับมุมมอง Overview, Follow และ Robot POV เรียลไทม์";
  } else if (currentPhase === 3) {
    activeImage = "assets/current-capture/07_mission_hold_uncertain.png";
    statusBadge = "HOLD_UNCERTAIN";
    statusColor = "#ffb703";
    calloutText = "AI ไม่มั่นใจ ระบบหยุด ไม่เดา (Confidence < 0.55 ➔ HOLD_UNCERTAIN)";
  } else {
    if (frame < 900) {
      activeImage = "assets/current-capture/09_mission_slow_down.png";
      statusBadge = "SLOW_DOWN (50 cm)";
      statusColor = "#ffb703";
      calloutText = "พบสิ่งกีดขวางระยะ 50 ซม. ➔ ชะลอความเร็ว (SLOW_DOWN 50%)";
    } else {
      activeImage = "assets/current-capture/10_mission_safe_stopped.png";
      statusBadge = "SAFE_STOPPED (20 cm)";
      statusColor = "#ef233c";
      calloutText = "หยุดปลอดภัย และรอผู้ควบคุม (SAFE_STOP ล็อคการทำงานจนกว่าจะ Manual Resume)";
    }
  }
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0b0f19", color: "#ffffff" }}>
      ...
      {/* Main Container for Screenshot Display */}
      <div
        style={{
          position: "absolute",
          top: 130,
